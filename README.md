...
## 📄 st. Requemit a Pullsub to freease feel Plere welcome! ions abutntriting

CobuntriCo

## 🤝 ile
``` # This f             ME.md s
└── READl servicetop al  # S        l.sh   op_alst
├── ervicestart all s   # Sh           _all.sun
├── rciesn dependen # Pythot   nts.txiremequ
│   └── resnt for studeastAPI apppy      # F_app.── studenttion
│   ├inier def Contain     #kerfile     oc── D│   ├ntainer
nt pod co  # Stude         tudent_pod/ ncies
├── sende # Node dep   son    kage.j└── pacyles
│   ind st    # Tailwindex.css   ─ │   │   └─nent
in UI compo Ma      #─ App.jsx   │   │   ├─ src/
d
│   ├──enntfroct      # Reaui/        student-ncies
├── hon dependet    # Pyttxrements. requis
│   └──a model    # Dat       pys.del├── moations
│   MongoDB oper        # y          ├── db.ptions
│raernetes ope       # Kubpy       ├── k8s.ion
│   icat appl  # Main API            pp.py a
│   ├──ackendPI b # FastA         nd/       cke
├── baloyer/d-depdent-po
...
## 📊 API 
```
🎓"}t pod!  your studenello from "H":turn {"msgreome():
    f h
deet("/")
@app.ghon```pyt.py`:
...
## 🔧  Kubernetatus with to sync s"Refresh"
...
─────   ┌────         ↓
      ...
      ┘────┬─────────  └─┘────)  │
└────ikube  │  │  (Min
│ Atlas es  │ Kubernet│ B │  oD───┐
│ Mong─────────  ┌──┌─────────┐   ↓
  
     ↓        │
     │   ──┘───────┬──┬─  │
└───)  t 8000Por│  (thon)
stAPI + Py  │  (Faend API ck┐
│   Ba────────────── ↓
┌───
        
         │┬────────┘─────
└───   │5173) Port S)
│  (wind CSTailact + UI    │  (ReStudent ─────┐
│   ───────────
```
┌─
itecture
